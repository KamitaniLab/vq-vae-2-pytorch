...
def save_videos(
    ...
    **kwargs
    ...
    f_name_base =f_name_base.replace('sample_frame', 'sample_video')
    os.makedirs('sample_video', exist_ok=True)
        ...
        if dist.is_primary():
            ...
            if i % 100 == 0:
                ...
                save_videos(
                    ...
                    f"sample_frame/{str(epoch + 1).zfill(5)}_{str(i).zfill(5)}.png",
                    nrow=sample_size,
                    normalize=True,
                    range=(-1, 1),
                )
...
def main(args):
    ...
    for i in range(args.epoch):
        ...
        if dist.is_primary():
            torch.save(model.state_dict(), f"checkpoint_vid/vqvae_{str(i + 1).zfill(3)}.pt")
