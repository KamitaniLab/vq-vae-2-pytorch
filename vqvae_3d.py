    ...
    def __init__(self, in_channel, channel, n_res_block, n_res_channel, stride):
        ...
        if stride == 4:
            blocks = [
                ...
                nn.ReLU(inplace=True),
                nn.Conv3d(channel // 2, channel, 4, stride=(1,2,2), padding=1),
                nn.ReLU(inplace=True),
                nn.Conv3d(channel, channel, (2,3,3), padding=1),
            ]
...
class Decoder(nn.Module):
    def __init__(
        ...
        if stride == 4:
            ...
            blocks.append(
                nn.ConvTranspose3d(channel, out_channel, 4, stride=2, padding=(1,1,1))
            )
...
if __name__ == '__main__':
    ...
    #rand
    input_quant = torch.randn(1, 128, 8, 64, 64)
